...
function getNodeFromStrOffset($offset,$doc){
	$doc->textContent;
...
function getNodeForOffset(&$root,$start,$offset){
	
	$txtcon = $root->textContent;
	echo substr($txtcon,0,20)."<br/><br/>";
	$curLen = strlen($txtcon);
	echo "current length $curLen <br/>";
 	$jump = $start+$curLen;	
	if (($jump>=$offset) && ($start<=$offset)){
		echo "in here $offset $start $jump<br/>";
		$allKids= $root->childNodes;
		$first = $root->firstChild;
		echo "First child is ".$first->nodeName;
		echo get_class($first);
		if (($allKids->length==1)&&(get_class($first) == "DOMText")){
			echo "GOOOOOO<br/>";
			return $root;
		}
		else{
			$i=0;
			while (get_class($first) == "DOMText"){
				echo "Looping ";
				$i++;
				$start = $start+$first->length;
				$first = $root->childNodes->item($i);
				if ($first == null)	{
					echo "huh";
				}
			}
			
			echo "new start: $start<br/>";
			return getNodeForOffset($first,$start,$offset);
		...
		if ($start<=$offset){
			...
			{
				echo "should be ok";
			$nextSib = $root->nextSibling;
			echo "JUMP from $start to $jump at $nextSib->nodeName<br/>";
			return getNodeForOffset($nextSib,$jump,$offset);
			}
			
		}	
		echo "not $start $offset $jump";
